...
def smooth_data(n, loc, scale, dim=3):
    ...
    if dim == 1:
        return SpatialPointVector(stats.norm.rvs(size=n, loc=loc, scale=scale), dim=dim)
...
def jagged_data(n, loc, scale, dim=3):
    ...
    if dim == 1:
        return SpatialPointVector(stats.uniform.rvs(size=n, loc=loc, scale=scale), dim=dim)
